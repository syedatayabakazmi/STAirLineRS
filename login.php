...
if (isset($_SESSION['username'])) {
    ...
    if ($_POST["sign"] == "login") {
        ...
        // Define variables for save in database
        ...
        if (empty($name)) {
            ...
            // Check if email exist in user's table in db
            if (empty($email)) {
                ...
                if ($num >= 1) {
                    ...
                    // Check if username exist in user's table in db
                    if (empty($username)) {
                        $showError = "Username cannot be Blank";
                    } elseif (isset($username)) {
                        $sql = "select id from userTbl where username = '$username' ";
                        $stmt = mysqli_query($con, $sql);
                        $num = mysqli_num_rows($stmt);
                        if ($num >= 1) {
                            ...
                            if (empty($password)) {
                                ...
                                // Chech password match or not in both field
